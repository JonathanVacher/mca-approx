...
class BaseGluing:
    ...
    def compute_error_bounds(self) -> Tuple[float, float]:
        ...
        weak = (
            ...
            * cfg.h**2
            * (
                (cfg.sig**2 / 8) * safe_max(self.pfd4xx)
                + (1 / (6 * cfg.tau)) * safe_max(self.pfd4xx)
            )
        )

        lam = cfg.sig**2
        Bmax = max(abs(cfg.x_safe_min), abs(cfg.x_safe_max)) / cfg.tau

        density = (
            1
            / np.sqrt(2 * np.pi * lam * cfg.t_max)
            * np.exp(Bmax**2 * cfg.t_max / (2 * lam))
        )

        smoothing = density * cfg.epsilon / 2

        return float(weak), float(smoothing)
