...
class SDEConfig:
    ...
    def drift_step(self, x: float, u: Optional[float] = None) -> float:
        """Compute one Euler-Maruyama displacement."""
        if self.is_controlled:
            if u is None:
                raise ValueError("A control value u is required for controlled dynamics.")
            drift = self.mu(x, u)
        else:
            drift = self.mu(x)
        diffusion = self.sigma(x)
        noise = diffusion * np.sqrt(self.dt) * np.random.randn()
        return drift * self.dt + noise
