...
def _time_grid_steps(config) -> int:
    n_steps = int(round(config.t_max / config.dt))
    if not np.isclose(n_steps * config.dt, config.t_max):
        raise ValueError("config.t_max must be an integer multiple of config.dt for MDP time bounds.")
    return n_steps
...
def generate_prism_mdp_model(config, simulator) -> str:
    ...
    h = config.h
    n_states = simulator.n_states
    x_min = float(simulator.x_space[0])
...
def generate_pctl(config) -> str:
    ...
    """
        ...
        t_steps = _time_grid_steps(config)
        return f"""
    const int T_steps = {t_steps};

    // Find the strategy that MAXIMIZES the probability of staying safe
    // MDP time is counted in discrete steps of length config.dt.
    Pmax=? [ G<=T_steps "safe" ]
