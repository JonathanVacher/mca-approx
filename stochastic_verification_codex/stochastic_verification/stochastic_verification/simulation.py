...
class StochasticSimulator:
    ...
    def __init__(self, config: SDEConfig):
        ...
        self.start_idx = int(np.argmin(np.abs(self.x_space - self.config.x_init)))
    ...
    def run_monte_carlo(self, n_reps: int = 1000) -> Dict[str, np.ndarray]:
        ...
        if not np.isclose((n_steps - 1) * dt, t_max):
            raise ValueError("config.t_max must be an integer multiple of config.dt.")
        exp_dt_qs = []
        if not self.config.is_controlled:
            exp_dt_q = linalg.expm(dt * self.q_matrix).T
        else :
